...
@section('page-header')
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="page-title">{{ __('Create Accrual Plan') }}</h3>
                <ul class="breadcrumb">
...
@section('page-content')
    <div class="content container-fluid">
        ...
        <form action="{{ route('leave.config.accrual-plans.store') }}" method="POST" id="accrualPlanForm">
            @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="accrued_gain_time" id="accrued_gain_time" class="form-select"
                                ...
                                <option value="start">{{ __('At the start of the accrual period') }}</option>
                                <option value="end">{{ __('At the end of the accrual period') }}</option>
                            </select>
                        </div>
                        <div class="col-md-3 mb-3">
                            ...
                            <select name="transition_mode" class="form-select">
                                ...
                                <option value="after_accrual">{{ __('After next accrual') }}</option>
                            </select>
                        </div>

                        <div class="col-md-12 mb-3" id="worked_time_container" style="display: none;">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_based_on_worked_time"
                                    id="is_based_on_worked_time" value="1" {{ old('is_based_on_worked_time') ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="carry_over_time" class="form-select" id="carry_over_time"
                                ...
                                <option value="year_start">{{ __('At the beginning of the year (Jan 1)') }}</option>
                                <option value="allocation">{{ __('At the allocation date') }}</option>
                                <option value="other">{{ __('At a custom date') }}</option>
                            </select>
                        </div>

                        <div id="custom_carryover_date" class="col-md-8 row" style="display: none;">
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="carry_over_day" class="form-control" min="1" max="31" value="1">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div id="milestones_container">
                        {{-- Initialize with one milestone if old() is empty --}}
                        @if(old('levels'))
                            @foreach(old('levels') as $idx => $oldLevel)
                                @include('leave.configuration.accrual-plans.partials.milestone_item', ['index' => $idx, 'level' => (object) $oldLevel])
                            @endforeach
                        @endif
                    </div>
            ...
            <div class="d-flex justify-content-end gap-2 mb-5">
                ...
                <button type="submit" class="btn btn-primary btn-lg px-5">{{ __('Save Accrual Plan') }}</button>
            </div>
        </form>
    </div>
        ...
        <script>
            {
                ...
                function updateMilestoneSequences() {
                    ...
                    items.forEach((item, index) => {
                        ...
                        item.querySelector('.milestone-seq').value = seq;
                    });
                }
