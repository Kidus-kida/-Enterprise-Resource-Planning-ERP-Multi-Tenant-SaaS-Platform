            ...
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('shifts.rotation.store') }}" method="POST" id="rotationForm">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                                               value="{{ old('name') }}" placeholder="{{ __('e.g., Weekly Day/Night Swap') }}" required>
                                        @error('name')
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <select name="frequency_type" class="form-select @error('frequency_type') is-invalid @enderror" required>
                                            <option value="daily" {{ old('frequency_type') == 'daily' ? 'selected' : '' }}>{{ __('Daily') }}</option>
                                            <option value="weekly" {{ old('frequency_type', 'weekly') == 'weekly' ? 'selected' : '' }}>{{ __('Weekly') }}</option>
                                            <option value="monthly" {{ old('frequency_type') == 'monthly' ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                                        </select>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <small class="text-muted">{{ __('The cycle will start from this date.') }}</small>
                                        @error('start_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div id="shiftsContainer">
                                @php $oldShifts = old('shifts', [null]); @endphp
                                @foreach($oldShifts as $index => $oldId)
                                        ...
                                        <div class="flex-grow-1">
                                            <select name="shifts[]" class="form-select @error('shifts.'.$index) is-invalid @enderror" required>
                                                ...
                                                @foreach($shifts as $shift)
                                                    <option value="{{ $shift->id }}" {{ $oldId == $shift->id ? 'selected' : '' }}>
                            ...
                            <div class="submit-section mt-4">
                                <button class="btn btn-primary submit-btn w-100" type="submit">{{ __('Save Rotation Plan') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Update interval label based on frequency type
