        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="input-block mb-3">
                    ...
                    <x-form.input type="text" id="award_type" name="award_type" value="{{ old('award_type') }}" placeholder="{{ __('e.g. Best Employee, performace, Leadership') }}"
                        required />
                    @error('award_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="col-md-6">
                <div class="input-block mb-3">
                    ...
                    <select name="user_id" id="user_id" class="select form-control @error('user_id') is-invalid @enderror"
                        required>
                        <option value="">{{ __('Select User') }}</option>
                        @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id')==$user->id ? 'selected' : '' }}>{{
                            $user->fullname ?? $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="col-md-6">
                <div class="input-block mb-3">
                    ...
                    <x-form.input type="text" id="awarded_by" name="awarded_by" 
                        class="form-control @error('title') is-invalid @enderror" value="{{ old('awarded_by') }}" required/>
            ...
            <div class="col-12">
                <div class="input-block mb-3">
                    ...
                    <x-form.input type="file" id="certificate" name="certificate" accept=".pdf,.jpg,.jpeg,.png" />
                </div>
            </div>
        </div>
