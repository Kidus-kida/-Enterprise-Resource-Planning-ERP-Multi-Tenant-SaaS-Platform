...
class ContactUtil
{
    ...
    public $supplier_types = ['cheque_return','property_purchase','expense','opening_balance','purchase','purchase_return'];
    ...
    public function getSupplierBalance($contact_id,$business_id,$get_balance = false){
        ...
        if(!empty($txns)){
            ...
            $balance_details['total_purchase'] = ($txns->cheque_return + $txns->property_purchase +  $txns->expense + $txns->purchase) - ($txns->purchase_return + $txns->ledger_discount);
    ...
    public function getSupplierLedger($contact_id,$business_id,$start_date,$end_date){
        ...
        $txnResult = $txns->unionAll($pmts)->orderBy('date', 'asc');
