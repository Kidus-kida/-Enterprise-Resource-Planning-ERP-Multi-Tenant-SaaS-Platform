...
class DepositsController extends Controller
{
    ...
    {
        $business_id = auth()->user()->business_id; // session()->get('user.business_id');
        ...
        $account_groups_raw = AccountGroup::get()->toArray();
        $account_groups = [];
        ...
        $users = User::where('business_id', $business_id)->get()->pluck('name', 'id');
        $orderStatuses = $this->productUtil->orderStatuses();
        $suppliers = Contact::suppliersDropdown($business_id, false);
        $customers = Contact::customersDropdown($business_id, false);
        $chequeId = $this->transactionUtil->account_exist_return_id('Cheques in Hand');
        $asset_v = 1; // Default asset version
        return view('deposits.index')
            ->with(compact('chequeId', 'customers', 'parentAccounts', 'filterdata', 'account_types_opts', 'sub_acn_arr', 'not_linked_payments', 'account_types', 'account_access', 'business_locations', 'account_groups', 'users', 'accounts', 'suppliers', 'orderStatuses', 'asset_v'));
    }
    /**
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->addColumn('debit', function ($row) use ($business_details, $currency_precision) {
                    ...
                    if (!empty($row->type)) {
                        $total_daily_collection = 0;
                        // Removed Petro module Settlement/DailyCollection logicip
                ...
                ->addColumn('description', function ($row) use ($business_id, $id, $currency_precision) {
                    ...
                    if ($row->transaction && $row->transaction->type == 'property_purchase') {
                        ...
                        // Default logic if needed or empty
                        $details = '';
                    }
    ...
    public function getDeposit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            if ($type == 'cash') {
                ...
                if (empty($account)) {
                    return response()->json(['error' => 'Cash account not found. Please create a Cash account first.'], 404);
                }
                $id = $account->id;
            } elseif ($type == 'card') {
    ...
    public function account_access($business_id)
    {
        ...
        // Replaced logic to avoid Superadmin Subscription class dependency
        $account_access = 0;
        // You might want to use ModuleUtil here if available or just return 0/1 based on some other logic. 
        // For now, defaulting to 0 or 1 based on user permission if needed, 
        // but the original code checked a specific package permission.
        // We will assume no access via this specific check unless we have ModuleUtil available in this context.
        // Actually, $this->moduleUtil IS available in the controller.
        // So we can use $this->moduleUtil->hasThePermissionInSubscription($business_id, 'account_access');

        // However, this function is static-like but it uses $this? No it is an instance method.
        if (isset($this->moduleUtil)) {
            $account_access = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'access_account') ? 1 : 0;
        }
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->editColumn('sub_type', function ($row) {
                    ...
                    return __('deposits.' . $row->sub_type);
                })
    ...
    public function checkAccountNumber(Request $request)
    {
        ...
        if ($count > 0) {
            return response()->json(['success' => false, 'msg' => __('lang_v1.account_number_already_exists')]);
        }

        return response()->json(['success' => true, 'msg' => 'Success']);
    }

    public function checkAccountNames(Request $request)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        $accounts = $query->select('id', 'name as text')->get();
