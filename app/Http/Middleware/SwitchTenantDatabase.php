...
class SwitchTenantDatabase
{
    ...
    {
        if ($request->is('superadmin*') || $request->is('diagnostic*') || $request->is('logout') || $request->is('login*') || $request->is('register*') || $request->is('forgot-password*') || $request->is('reset-password*') || $request->is('tenant-debug*') || $request->path() === 'tenant-debug' || \Illuminate\Support\Str::startsWith($request->path(), 'tenant-debug/') || $request->is('tenant/*') || $request->is('tenant/*/*')) {
            return $next($request);
        }
