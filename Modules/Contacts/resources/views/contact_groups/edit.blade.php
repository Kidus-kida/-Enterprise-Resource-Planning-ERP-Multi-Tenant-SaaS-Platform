<div class="modal-body">
    <form action="{{ route('contact-groups.update', $contact_group->id) }}" method="post" id="contact_group_edit_form">
        @csrf
        @method('PUT')
        <div class="row">
           <div class="col-md-12">
                ...
                <div class="input-block mb-3">
                    ...
                    <select class="form-control select" name="type" id="type">
                        @foreach($types as $key => $value)
                            <option value="{{ $key }}" @if($contact_group->type == $key) selected @endif>{{ $value }}</option>
                        @endforeach
                    </select>
                </div>
           </div>
                ...
                <div class="input-block mb-3">
                    ...
                    <select name="account_type_id" class="form-control select" id="changeAccountSelect">
                        ...
                        @foreach($allAccountsType as $accountType)
                            <option value="{{ $accountType->id }}" @if($contact_group->account_type_id == $accountType->id) selected @endif>{{ $accountType->name }}</option>
                        @endforeach
                    </select>
                </div>
           </div>
                    ...
                    <select name="interest_account_id" class="form-control select" id="AccountName">
                        ...
                        @if($allAccounts)
                            @foreach($allAccounts as $account)
                                 <option value="{{ $account->id }}" @if($contact_group->interest_account_id == $account->id) selected @endif>{{ $account->name }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
