<div class="modal-dialog modal-lg" role="document">
  <div class="modal-content">
    ...
    <form action="{{ action([\Modules\Products\Http\Controllers\CategoryController::class, 'update'], [$category->id]) }}"
      method="post" id="category_edit_form">
      @csrf
      @method('PUT')
      ...
      <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="short_code" value="{{ $category->short_code }}" class="form-control"
            ...
            <div class="mb-3">
              <div class="form-check">
                <input type="checkbox" name="add_as_sub_cat" value="1" class="form-check-input toggler"
                  data-toggle_id="parent_cat_div" data-toggle_class="parent_cat_div" id="add_as_sub_cat" @if(!$is_parent)
                  checked @endif>
          ...
          @if($account_access)
            <div class="mb-3 add_related_account">
              ...
              <select name="add_related_account" id="add_related_account" class="form-select select2" style="width: 100%">
                <option value="">Please Select</option>
                <option value="category_level" @if($category->add_related_account == 'category_level') selected @endif>Category
                  Level</option>
                <option value="sub_category_level" @if($category->add_related_account == 'sub_category_level') selected @endif>Sub
            ...
            <div class="mb-3">
              <div class="form-check">
                <input type="checkbox" name="weight_excess_loss_applicable" value="1" class="form-check-input toggler"
                  ...
                  @if($category->weight_excess_loss_applicable) checked @endif>
                <label class="form-check-label" for="weight_excess_loss_applicable">Weight Loss Excess Applicable</label>
              </div>
            </div>

            <div
              class="mb-3 weight_loss_expense_account weight_excess_loss_applicable_field @if(!$category->weight_excess_loss_applicable) d-none @endif">
              ...
              <select name="weight_loss_expense_account_id" class="form-select select2" style="width: 100%">
                ...
                @foreach($expense_accounts as $id => $name)
                  <option value="{{ $id }}" @if($id == $category->weight_loss_expense_account_id) selected @endif>{{ $name }}
                  </option>
                @endforeach
              </select>
            </div>
            ...
            <div
              class="mb-3 weight_excess_income_account weight_excess_loss_applicable_field  @if(!$category->weight_excess_loss_applicable) d-none @endif">
              ...
              <select name="weight_excess_income_account_id" class="form-select select2" style="width: 100%">
                ...
                @foreach($income_accounts as $id => $name)
                  <option value="{{ $id }}" @if($id == $category->weight_excess_income_account_id) selected @endif>{{ $name }}
                  </option>
                @endforeach
              </select>
            </div>
          ...
          <div class="mb-3 vat_fields" @if($category->vat_exempted == 'Yes') style="display:none;" @endif>
            ...
            <select name="vat_based_on" class="form-select select2 vat_based_on">
              <option value="">Please Select</option>
              <option value="vat_not_applicable" @if($category->vat_based_on == 'vat_not_applicable') selected @endif>VAT Not
                Applicable</option>
              <option value="sale_price" @if($category->vat_based_on == 'sale_price') selected @endif>Sale Price</option>
              <option value="profit" @if($category->vat_based_on == 'profit') selected @endif>Profit</option>
              <option value="profit_percentage" @if($category->vat_based_on == 'profit_percentage') selected @endif>Profit
                Percentage</option>
            </select>
          </div>

          <div class="mb-3 vat_fields" @if($category->vat_exempted == 'Yes') style="display:none;" @endif>
            <label for="apply_vat_on" class="form-label">@lang('category.apply_vat_on'):*</label>
            <select name="apply_vat_on" class="form-select select2">
              <option value="">Please Select</option>
              <option value="on_product_sub_category_settings" @if($category->apply_vat_on == 'on_product_sub_category_settings') selected @endif>@lang('category.on_product_sub_category_settings')</option>
              <option value="on_product_tax_settings_section" @if($category->apply_vat_on == 'on_product_tax_settings_section') selected @endif>@lang('category.on_product_tax_settings_section')</option>
            </select>
          </div>

          <div class="mb-3 vat_fields profit_percentage" @if($category->vat_based_on != 'profit_percentage')
          style="display:none;" @endif>
            <label for="profit_percentage" class="form-label">Profit Percentage:*</label>
            <input type="text" name="profit_percentage" value="{{ $category->profit_percentage }}"
              class="form-control profit_percentage" placeholder="Profit Percentage">
          </div>
...
<script>
  ...
  // Re-bind toggler for Add/Edit as sub-category
